...
router.get("/me", async (req, res) => {
  ...
  return res.json({
    id: userId,
    email: req.interpreterUser.email,
    profile: profileResult.data,
    membership: entitlementResult.data || { plan_id: "free", status: "active" },
    notifications: preferencesResult.data
  });
});
...
router.patch("/me", async (req, res) => {
  ...
  const { data, error } = await admin
    .from("profiles")
    .upsert({ id: req.interpreterUser.id, full_name: fullName, phone }, { onConflict: "id" })
...
router.delete("/me", async (req, res) => {
  const admin = getSupabaseAdmin();
  const { error } = await admin.auth.admin.deleteUser(req.interpreterUser.id);
