...
router.post("/:callId/answer", async (req, res) => {
  ...
  try {
    ...
    const bridge = new TranslatorBridge(call, process.env.LIVEKIT_URL, bridgeToken);
    await bridge.start();
    call.bridge = bridge;
    call.status = "active";
    const participantToken = await createVoiceToken({ identity: call.recipientIdentity, roomName });
    return res.json({ call: publicCall(call), serverUrl: process.env.LIVEKIT_URL, participantToken });
  } catch (error) {
