...
export function VoiceCallHost() {
  useEffect(() => {
    void VoiceCallService.resetVoiceCall({ notifyBackend: false });
    ...
    const responseListener = Notifications.addNotificationResponseReceivedListener((response) => handleIncoming(response.notification));
    const appStateListener = AppState.addEventListener('change', (state) => {
      if (state === 'active') void VoiceCallService.handleAppForeground();
    });
    void Notifications.getLastNotificationResponseAsync().then((response) => handleIncoming(response?.notification));
    return () => {
