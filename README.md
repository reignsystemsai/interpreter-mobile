...
membership, notification, and usage data are isolated under versioned `/api/v1`
modules so they can scale independently from live interpretation.

RevenueCat-backed membership features are configured through the app's server-side
values and the webhook at `POST /api/v1/subscriptions/revenuecat/webhook`.
...
The app continues to use the direct LiveKit voice flow for interpreted conversations.
Any call metadata or account state is handled by the app's own backend services and
must not rely on Supabase.
